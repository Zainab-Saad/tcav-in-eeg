...
def get_estimator(data_shape, class_weights, valid_sampler, lr, max_lr, output_dir, run):
    net = NeuralNetClassifier(module=XceptionTimePlus, module__c_in=data_shape, module__c_out=2,
                              ...
                              batch_size=32, lr=lr, callbacks=[EpochScoring("accuracy",
                                                                  ...
                                                                  LRScheduler(policy=optim.lr_scheduler.OneCycleLR,
                                                                              monitor="valid_loss", max_lr=max_lr,
                                                                              epochs=1,
                                                                              steps_per_epoch=100,
                                                                              div_factor=10,
...
max_lrs = [0.000005, 0.00005, 0.0005, 0.005, 0.0001, 0.005]
...
for lr in learning_rates:
    for max_lr in max_lrs:
        ...
        run_name = f"onecycleLR_{lr}_{max_lr}"
        output_dir = os.path.join("experiments", run_name)
        os.makedirs(output_dir, exist_ok=True)
        
        # MLflow run
        tags = {"script_name": os.path.basename(__file__), "lr": lr, "max_lr": max_lr}
        run = client.create_run(experiment.experiment_id, run_name=run_name, tags=tags)

        print(f"\nStarting run: {run_name}")
        estimator = get_estimator(data_shape, class_weights, valid_sampler, lr, max_lr, output_dir, run)
